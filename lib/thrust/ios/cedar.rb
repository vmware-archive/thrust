      ...
      def run(build_configuration, target, runtime_sdk, build_sdk, device, device_type_id, build_dir, simulator_binary)
        if build_sdk == 'macosx'
          ...
          if simulator_binary =~ /waxim%/
            ...
            device_type_id_param_string = nil
            unless device_type_id.nil?
              device_type_id_param_string = %Q[--devicetypeid #{device_type_id} ]
            end
            if device == "ipad"
              @thrust_executor.check_command_for_failure(%Q[#{simulator_binary} launch #{app_executable} #{device_type_id_param_string}--sdk #{runtime_sdk} --family #{device} --setenv CFFIXED_USER_HOME=#{Dir.tmpdir} --setenv CEDAR_HEADLESS_SPECS=1 --setenv CEDAR_REPORTER_CLASS=CDRDefaultReporter])
            else
              @thrust_executor.check_command_for_failure(%Q[#{simulator_binary} launch #{app_executable} #{device_type_id_param_string}--sdk #{runtime_sdk} --family #{device} --retina --tall --setenv CFFIXED_USER_HOME=#{Dir.tmpdir} --setenv CEDAR_HEADLESS_SPECS=1 --setenv CEDAR_REPORTER_CLASS=CDRDefaultReporter])
            end
          else
