...
describe Thrust::Git do
  ...
  describe '#commit_summary_for_last_deploy' do
    context 'when the target has been deployed previously' do
      it 'uses the commit message from that commit' do
        Thrust::Executor.should_receive(:capture_output_from_system).with('autotag list staging').and_return("7342334 ref/blah\nlatest_deployed_commit ref/blahblah")
        Thrust::Executor.should_receive(:capture_output_from_system).with("git log --oneline -n 1 latest_deployed_commit").and_return('summary')
    ...
    context 'when the target has not been deployed' do
      it 'says that the target has not been deployed' do
        Thrust::Executor.should_receive(:capture_output_from_system).with('autotag list staging').and_return("\n")
        summary = git.commit_summary_for_last_deploy('staging')
  ...
  describe '#generate_notes_for_deployment' do
    ...
    it 'generates deployment notes from the commit log history' do
      Thrust::Executor.should_receive(:capture_output_from_system).with('git rev-parse HEAD').and_return("latest_commit\n")
      Thrust::Executor.should_receive(:capture_output_from_system).with('autotag list staging').and_return("7342334 ref/blah\nlatest_deployed_commit")
      Thrust::Executor.should_receive(:system_or_exit).with('git log --oneline latest_deployed_commit...latest_commit', temp_file.path)
    ...
    context 'when there are no previously deployed commits' do
      it 'returns the commit message of just the latest commit' do
        Thrust::Executor.should_receive(:capture_output_from_system).with('git rev-parse HEAD').and_return("latest_commit\n")
        Thrust::Executor.should_receive(:capture_output_from_system).with('autotag list staging').and_return("\n")
        Thrust::Executor.should_receive(:capture_output_from_system).with("git log --oneline -n 1 latest_commit").and_return('summary')
