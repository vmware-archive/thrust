...
describe Thrust::Android::Deploy do
  ...
  describe ".make" do
    ...
    it 'passes app_config, and distribution_config to the Thrust::Android::Deploy' do
      Thrust::Android::Deploy.should_receive(:new).with($stdout, anything, anything, anything, 'true', 'devs')
      make
    end
  ...
  describe "#run" do
    ...
    subject(:deploy) { Thrust::Android::Deploy.new(out, android_tools, git, testflight, 'true', 'devs') }
    ...
    it 'uploads to TestFlight' do
      testflight.should_receive(:upload).with('apk_path', 'true', 'devs')
      deploy.run
    end
